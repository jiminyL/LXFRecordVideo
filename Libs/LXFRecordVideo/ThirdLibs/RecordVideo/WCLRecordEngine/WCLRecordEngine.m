...
@property (nonatomic) NSInteger cx;
@property (nonatomic) NSInteger cy;
...
@property (atomic, assign) BOOL isFront;//是否前置摄像头
...
- (NSInteger)cx {
    NSInteger result = ceilf(_cx / 16) * 16;
    return result;
}

- (NSInteger)cy {
    NSInteger result = ceilf(_cy / 16) * 16;
    return result;
}
        ...
        if (self.isCapturing) {
            ...
            dispatch_async(_captureQueue, ^{
                [self.recordEncoder finishWithCompletionHandler:^{
                    if (mself.recordEncoder.writer.status != AVAssetWriterStatusFailed && self.recordEncoder.writer.status == AVAssetWriterStatusCompleted) {
                        mself.isCapturing = NO;
                        mself.recordEncoder.writer = nil;
                        mself.recordEncoder = nil;
                        mself.startTime = CMTimeMake(0, 0);
                        mself.currentRecordTime = 0;
                        if ([mself.delegate respondsToSelector:@selector(recordProgress:)]) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [mself.delegate recordProgress:mself.currentRecordTime/mself.maxRecordTime];
                            });
                        }
                        
                        [mself movieToImageHandler:handler];
                    }else {
                        if (mself.recordVideoError) {
                            mself.recordVideoError(mself.recordEncoder.writer.error.description);
                        }
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [mself resumeCapture];
                        });
...
- (AVCaptureSession *)recordSession {
    if (_recordSession == nil) {
        ...
        if ([_recordSession canAddOutput:self.videoOutput]) {
            ...
            //设置视频的分辨率
            CGFloat videoWidth = (kLXFScreenWidth * 3) * 0.2;
            _cx = videoWidth;
            if (self.videoConnection.videoOrientation == AVCaptureVideoOrientationLandscapeRight || self.videoConnection.videoOrientation == AVCaptureVideoOrientationLandscapeLeft) {
                if (kLXFScreenWidth >= kLXFScreenHeight) {
                    _cy = videoWidth * ((CGFloat)kLXFScreenWidth/kLXFScreenHeight);
                }else {
                    _cy = videoWidth * ((CGFloat)kLXFScreenHeight/kLXFScreenWidth);
                }
            }else {
                if (kLXFScreenHeight >= kLXFScreenWidth) {
                    _cy = videoWidth * ((CGFloat)kLXFScreenHeight/kLXFScreenWidth);
                }else {
                    _cy = videoWidth * ((CGFloat)kLXFScreenWidth/kLXFScreenWidth);
                }
            }
        }
        //添加音频输出
...
- (void)resizeOrientationWithAVCaptureVideoOrientation:(AVCaptureVideoOrientation)orientation
{
    ...
    self.recordOrientation = [LXFDeviceManager sharedInstance].deviceOrientation;
    
    if ([LXFDeviceManager sharedInstance].deviceOrientation == UIInterfaceOrientationLandscapeRight || [LXFDeviceManager sharedInstance].deviceOrientation == UIInterfaceOrientationLandscapeLeft) {
        CGFloat videoWidth = (kLXFScreenWidth * 3) * 0.2;
        _cx = videoWidth;
        ...
        CGFloat videoWidth = (kLXFScreenHeight * 3) * 0.2;
        _cx = videoWidth;
...
{
    self.videoConnection.videoOrientation = UIInterfaceOrientationLandscapeLeft;
}
...
- (AVCaptureDeviceInput *)backCameraInput {
    if (_backCameraInput == nil) {
        ...
        if (error) {
            NSLog(@"获取后置摄像头失败~");
            if (self.fetchBackCameraFailed) {
                self.fetchBackCameraFailed(error);
            }
        }
    }
...
- (AVCaptureDeviceInput *)frontCameraInput {
    if (_frontCameraInput == nil) {
        ...
        if (error) {
            NSLog(@"获取前置摄像头失败~");
            if (self.fetchFrontCameraFailed) {
                self.fetchFrontCameraFailed(error);
            }
        }
    }
...
- (AVCaptureDeviceInput *)audioMicInput {
    if (_audioMicInput == nil) {
       AVCaptureDevice *mic = [AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeAudio];
        ...
        if (error) {
            NSLog(@"获取麦克风失败~");
            if (self.fetchMicrophoneFailed) {
                self.fetchMicrophoneFailed(error);
            }
        }
    }
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            NSString *videoName = @"tempVideo.mp4"; // [self getUploadFile_type:@"video" fileType:@"mp4"];
